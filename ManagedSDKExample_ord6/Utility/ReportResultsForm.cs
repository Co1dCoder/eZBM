...
namespace ManagedSDKExample
    ...
    public partial class ReportResultsForm : Form
        ...
        public ReportResultsForm()
            {
            InitializeComponent();
            m_reportHtml += "<html><body style='font-family:arial;font-size: 13px;'>";         
            }

        public void Finished()
            {
            m_reportHtml += "</body></html>";
            reportViewer.DocumentText = m_reportHtml;
            this.Show();
            }
        ...
        public void AddHeader(string text)
            {
            m_reportHtml += "<h3 style='background:#eee; padding: 5px;'>" + text + "</h3>";
            }

        public void AddSubheader(string text)
            {
            m_reportHtml += "<h4 style='padding: 5px;'>" + text + "</h4>";

            }

        public void AddLine(string text)
            {
            m_reportHtml += text + NewLineCharacter;
            }
        ...
        public void PrintValueTable(List<string> headerValues, Dictionary<string, string> values)
            ...
            m_reportHtml += "<p><table style='border:1px solid #777; margin:10px;' width='90%'>";
            ...
            if(headerValues != null && headerValues.Count > 0)
                ...
                foreach (string header in headerValues)
                    m_reportHtml += string.Format(headerCell, header);
                m_reportHtml += "</tr>";                
                }
            ...
            foreach (KeyValuePair<string, string> pair in values)
                ...
                m_reportHtml += "<tr>";
                if(labelHeaders)
                    m_reportHtml += string.Format(labelCell, pair.Key);
                else
                    m_reportHtml += string.Format(valueCell, pair.Key);

                if (pair.Value.Contains("|"))
                    {
                    string[] multipleValues = pair.Value.Split('|');
                    foreach (string s in multipleValues)
                        m_reportHtml += string.Format(valueCell, s);
                    }
                else
                    m_reportHtml += string.Format(valueCell, pair.Value);

                m_reportHtml += "</tr>";
                }

            m_reportHtml += "</table></p>";
            }
        ...
        public void PrintValueTable(List<string> headerValues, List<string> values)
            ...
            foreach (string value in values)
                ...
                if (value.Contains("|"))
                    ...
                    m_reportHtml += string.Format(valueCell, value);
            ...
            foreach (ReportData data in reportData)
                ...
                switch (data.type)
                    ...
                    case ReportDataType.header:
                        m_reportHtml += string.Format(headerCell, data.data);
                        break;
                    case ReportDataType.label:
                        m_reportHtml += string.Format(labelCell, data.data);
                        break;
                    case ReportDataType.data:
                        m_reportHtml += string.Format(dataCell, data.data);
                        break;
                    default:
                        break;
                    }
