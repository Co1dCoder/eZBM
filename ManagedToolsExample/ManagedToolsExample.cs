 ...
 *   
 *   TOOLSEXAMPLE BLEND BLENDEDGE : Allow user to select the edge which needs to be blended
 *   and expect user to enter blend radius and radius and allow tangency preferences. Once input is received
 *   It creates the blend.
+--------------------------------------------------------------------------------------*/
...
/*--------------+---------------+---------------+---------------+---------------+------*/
        ...
        * @bsimethod                                                              Bentley Systems
/*--------------+---------------+---------------+---------------+---------------+------*/
        internal void StartBlendTool(int toolId)
        {
            // TestModifyEdgeTool.InstallNewInstance(toolId);
        }
    }
}
